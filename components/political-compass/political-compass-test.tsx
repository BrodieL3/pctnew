...
export function PoliticalCompassTest() {
  ...
  const handleSubmit = () => {
    ...
    const calculatePosition = () => {
      ...
      const economicScore =
        economicQuestions.reduce((sum, q) => sum + (completeResponses[q] || 50), 0) / economicQuestions.length
      ...
      const socialScore =
        socialQuestions.reduce((sum, q) => sum + (completeResponses[q] || 50), 0) / socialQuestions.length
        ...
        {questions.map((question, index) => (
          <div
            ...
            className={`min-h-[calc(100vh-8rem)] flex items-center justify-center p-4 transition-opacity duration-500 ${
              isAnimating && index !== currentQuestionIndex ? "opacity-0" : "opacity-100"
            }`}
          >
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-2">
            <div className="flex justify-between sm:justify-start gap-2">
              ...
              {currentQuestionIndex < questions.length - 1 && (
                <Button onClick={() => scrollToQuestion(currentQuestionIndex + 1)}>
                ...
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs">
                      <p>
                        You've answered {answeredQuestionsCount} of {questions.length} questions.
