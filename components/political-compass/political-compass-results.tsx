    ...
    const loadResults = () => {
      try {
        ...
        if (savedResults) {
          ...
          const allNeutral = Object.values(responses).every((val) => val === 50)
      ...
      <div className="absolute bottom-4 left-4 z-10 bg-background/80 backdrop-blur-sm p-4 rounded-lg border max-w-xs">
        <div className="space-y-4">
          {isNeutralResult && (
            <div className="bg-blue-50 text-blue-800 p-3 rounded-md mb-4">
              <p className="text-sm font-medium">
                This is a "True Neutral" result, representing a perfectly balanced position on all issues.
              </p>
            </div>
          )}
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Economic: {userPosition.economic.toFixed(1)} | Social: {userPosition.social.toFixed(1)}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-muted p-2 rounded-md text-center">
              ...
              <div className="text-lg font-bold">{userPosition.economicScore.toFixed(1)}%</div>
              <div className="text-xs text-muted-foreground">
                {userPosition.economic < 0 ? "Left" : userPosition.economic > 0 ? "Right" : "Center"}
              </div>
            </div>
            <div className="bg-muted p-2 rounded-md text-center">
              <div className="text-sm text-muted-foreground">Social</div>
              <div className="text-lg font-bold">{userPosition.socialScore.toFixed(1)}%</div>
              <div className="text-xs text-muted-foreground">
                {userPosition.social < 0 ? "Libertarian" : userPosition.social > 0 ? "Authoritarian" : "Center"}
              </div>
            </div>
          </div>
